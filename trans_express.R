...
### try with P. equorum transcriptome ###
pe <- read.csv("P.uni_tran_express.csv")
head(pe)

pe1 <- pe %>%  
  gather(label, value, 1:6) %>%
  drop_na(value)
pe1
write.csv(pe1,"p.uni_trans_express2.csv")
# made changes in excel to this file eg removed empties, X and added isotype, added the group numbers i wanted

pe2 <- read.csv("P.uni_trans_express2.csv") %>% 
  arrange(group)
head(pe2)



ps <- ggplot(pe2, aes(fill=Isotype, y=relative.expression, x= fct_inorder(tissue))) + ### fct_inorder(tissue) puts x axis in order of the factors position in list
  geom_bar(position="fill", stat="identity") +
  theme_classic() +
  theme(axis.text.x= element_text(angle = 90, vjust = 0.5, hjust=1)) +
  labs(title = "Relative isotype expression at different life stages", x="Sample type", y="Relative expression") +
...
ps
ggsave("P.uni_trans_express.jpeg")
